...
| `src/changes.ts` | Pure decisions: what changed, by name (snapshot diff plus output fallbacks). |
...
`node:test` + type stripping, no test deps; `npm test` runs `tests/*.test.ts` (the glob is node's own — it is quoted so no shell expands it, which needs Node 22+). Tests cover the pure change detection in `src/changes.ts`: `changedNames`, `selfUpdated`, `hasChanges`, `changedPackages`. The fs/spawn wrappers in `src/snapshot.ts` and `runUpdate` are intentionally untested.
